...
class Network:
    def __init__(self, params=None):
        ...
        if self.params["TRAIN"]["INITIALIZER"] == "XAVIER":
            ...
            init = tf.compat.v1.random_normal_initializer()
        ...
        with tf.compat.v1.name_scope("Data"):
            tf.compat.v1.disable_eager_execution() # added on 27.08.2020 to disable eager execution
            self.X = tf.compat.v1.placeholder(tf.float32, [None, None, None, None, None])
        with tf.compat.v1.name_scope("Labels"):
            self.Y_onehot = tf.compat.v1.placeholder(tf.float32, [None, 32, 32, 32, 2])

        pp = preprocessor.Preprocessor(self.X)
        X_preprocessed = pp.out_tensor
        n_batchsize = tf.shape(input=X_preprocessed)[0]
        ...
        if self.params["VIS"]["ENCODER_PROCESS"]:
            with tf.compat.v1.name_scope("misc"):
                feature_maps = tf.compat.v1.get_collection("feature_maps")
                fm_list = []
                for fm in feature_maps:
                    fm_slice = fm[0, 0, :, :, 0]
                    fm_shape = fm_slice.get_shape().as_list()
                    fm_slice = tf.pad(tensor=fm_slice, paddings=[[0, 0], [127-fm_shape[0], 0]])
                    fm_list.append(fm_slice)
                fm_img = tf.concat(fm_list, axis=0)
                tf.compat.v1.summary.image("feature_map_list", tf.expand_dims(
                    tf.expand_dims(fm_img, -1), 0))

        # recurrent_module
        print("recurrent_module")
        with tf.compat.v1.name_scope("Recurrent_module"):
            ...
            if isinstance(n_timesteps, int) and n_timesteps > 0:
                ...
                n_timesteps = tf.shape(input=X_preprocessed)[1]
                ...
                hidden_state, t = tf.while_loop(
                    cond=condition, body=body, loop_vars=(hidden_state, t))
        ...
        if self.params["VIS"]["DECODER_PROCESS"]:
            with tf.compat.v1.name_scope("misc"):
                feature_voxels = tf.compat.v1.get_collection("feature_voxels")
                fv_list = []
                for fv in feature_voxels:
                    fv_slice = fv[0, :, :, 0, 0]
                    fv_shape = fv_slice.get_shape().as_list()
                    fv_slice = tf.pad(tensor=fv_slice, paddings=[[0, 0], [32-fv_shape[0], 0]])
                    fv_list.append(fv_slice)
                fv_img = tf.concat(fv_list, axis=0)
                tf.compat.v1.summary.image("feature_voxel_list", tf.expand_dims(
        ...
        if self.params["TRAIN"]["OPTIMIZER"] == "ADAM":
            ...
            tf.compat.v1.summary.scalar("adam_learning_rate", optimizer._lr)
        else:
            optimizer = tf.compat.v1.train.GradientDescentOptimizer(
                learning_rate=self.params["TRAIN"]["GD_LEARN_RATE"])
            tf.compat.v1.summary.scalar("learning_rate", optimizer._learning_rate)
        ...
        with tf.compat.v1.name_scope("metrics"):
            Y = tf.argmax(input=self.Y_onehot, axis=-1)
            predictions = tf.argmax(input=self.softmax, axis=-1)
            acc, acc_op = tf.compat.v1.metrics.accuracy(Y, predictions)
            rms, rms_op = tf.compat.v1.metrics.root_mean_squared_error(
                self.Y_onehot, self.softmax)
            iou, iou_op = tf.compat.v1.metrics.mean_iou(Y, predictions, 2)
            self.metrics_op = tf.group(acc_op, rms_op, iou_op)

        tf.compat.v1.summary.scalar("accuracy", acc)
        tf.compat.v1.summary.scalar("rmse", rms)
        ...
        if self.params["MODE"] == "TEST":
            ...
            self.val_writer = tf.compat.v1.summary.FileWriter(
    ...
    def step(self, data, label, step_type):
        ...
        if step_type == "train":
            ...
            out = self.sess.run(fetches, feed_dict) #output 28.09.2020
            ...
            options = tf.compat.v1.RunOptions(trace_level=3)
    ...
    def save(self):
        ...
        model_builder = tf.compat.v1.saved_model.builder.SavedModelBuilder(
            cur_dir + "/model")
