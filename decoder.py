...
def conv_vox(vox, in_featurevoxel_count, out_featurevoxel_count, K=3, S=[1, 1, 1, 1, 1], D=[1, 1, 1, 1, 1], initializer=None, P="SAME"):
    with tf.compat.v1.name_scope("conv_vox"):
        ...
        tf.compat.v1.add_to_collection("feature_voxels", ret)
        ...
        if params["VIS"]["KERNELS"]:
            ...
            kern_2 = tf.transpose(a=kern_1, perm=[3, 0, 1, 2])
            kern_3 = tf.expand_dims(kern_2, -1)
            kern_4 = tf.concat(tf.unstack(kern_3, axis=1), axis=1)
            tf.compat.v1.summary.image("3d kernel", kern_4, max_outputs=image_count)
        ...
        if params["VIS"]["VOXEL_SLICES"]:
            ...
            tf.compat.v1.summary.image("vox_slices", vox_slice_5,
                             max_outputs=image_count)

        if params["VIS"]["FEATURE_VOXELS"]:
            tf.compat.v1.summary.tensor_summary("feature_voxels", ret[0, :, :, :, 0])
...
def unpool_vox(value):  # from tenorflow github board
    with tf.compat.v1.name_scope('unpool_vox'):
...
def relu_vox(vox):
    with tf.compat.v1.name_scope("relu_vox"):
...
def block_simple_decoder(vox, in_featurevoxel_count, out_featurevoxel_count, K=3, D=[1, 1, 1, 1, 1], initializer=None, unpool=False):
    with tf.compat.v1.name_scope("block_simple_decoder"):
        if initializer is None:
            init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
        else:
            init = initializer
...
def block_residual_decoder(vox, in_featurevoxel_count, out_featurevoxel_count, K_1=3, K_2=3, K_3=1, D=[1, 1, 1, 1, 1], initializer=None, unpool=False):
    with tf.compat.v1.name_scope("block_residual_decoder"):
        if initializer is None:
            init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
        else:
            init = initializer
...
class Residual_Decoder:
    def __init__(self, hidden_state, feature_vox_count=[128, 128, 128, 64, 32, 2], initializer=None):
        with tf.compat.v1.name_scope("Residual_Decoder"):
            if initializer is None:
                init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
            else:
                init = initializer
...
class Dilated_Decoder:
    def __init__(self, hidden_state, feature_vox_count=[128, 128, 128, 64, 32, 2], initializer=None):
        with tf.compat.v1.name_scope("Dilated_Decoder"):
            if initializer is None:
                init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
            else:
                init = initializer
...
class Simple_Decoder:
    def __init__(self, hidden_state, feature_vox_count=[128, 128, 128, 64, 32, 2], initializer=None):
        with tf.compat.v1.name_scope("Simple_Decoder"):
            if initializer is None:
                init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
            else:
                init = initializer
