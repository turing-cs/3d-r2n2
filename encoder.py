...
def conv_sequence(sequence, in_featuremap_count, out_featuremap_count, initializer=None, K=3, S=[1, 1, 1, 1], D=[1, 1, 1, 1], P="SAME"):
    with tf.compat.v1.name_scope("conv_sequence"):
        ...
        def conv2d(x): return tf.nn.bias_add(tf.nn.conv2d(
            input=x, filters=kernel, strides=S, padding=P, dilations=D, name="conv2d"), bias)
        ret = tf.map_fn(conv2d, sequence, name="conv2d_map")

        tf.compat.v1.add_to_collection("feature_maps", ret)
        ...
        if params["VIS"]["KERNELS"]:
            ...
            kern_2 = tf.transpose(a=kern_1, perm=[2, 0, 1])
            kern_3 = tf.expand_dims(kern_2, -1)
            tf.compat.v1.summary.image("2d kernel", kern_3, max_outputs=image_count)
        ...
        if params["VIS"]["FEATURE_MAPS"]:
            ...
            tf.compat.v1.summary.image("feature_map", feature_map_3,
                             max_outputs=image_count)

        if params["VIS"]["HISTOGRAMS"]:
            tf.compat.v1.summary.histogram("kernel", kernel)
...
def max_pool_sequence(sequence, K=[1, 2, 2, 1], S=[1, 2, 2, 1], P="SAME"):
    with tf.compat.v1.name_scope("max_pool_sequence"):
        def max_pool(a): return tf.nn.max_pool2d(input=a, ksize=K, strides=S, padding=P)
        ret = tf.map_fn(max_pool, sequence, name="max_pool_map")
    return ret


def relu_sequence(sequence):
    with tf.compat.v1.name_scope("relu_sequence"):
...
def flatten_sequence(sequence):
    with tf.compat.v1.name_scope("flatten_sequence"):
        ret = tf.map_fn(
            #tf.contrib.layers.flatten,  sequence, name="flatten_map")
            #keras.layers.Flatten ,  sequence, name="flatten_map") # converted 29.08.20 to tf 2.0 by using keras.layers.Flatten
            tf.compat.v1.layers.flatten,  sequence, name="flatten_map") # converted 29.08.20 to tf2.0 
    return ret


def block_simple_encoder(sequence, in_featuremap_count, out_featuremap_count,  K=3, D=[1, 1, 1, 1], initializer=None):
    with tf.compat.v1.name_scope("block_simple_encoder"):
        if initializer is None:
            init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
        else:
            init = initializer
...
def block_residual_encoder(sequence, in_featuremap_count, out_featuremap_count,  K_1=3, K_2=3, K_3=1, D=[1, 1, 1, 1], initializer=None, pool=True):
    with tf.compat.v1.name_scope("block_residual_encoder"):
        if initializer is None:
            init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
        else:
            init = initializer
...
class Simple_Encoder:
    def __init__(self, sequence, feature_map_count=[96, 128, 256, 256, 256, 256], initializer=None):
        with tf.compat.v1.name_scope("Simple_Encoder"):
            if initializer is None:
                init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
            else:
                init = initializer
...
class Residual_Encoder:
    def __init__(self, sequence, feature_map_count=[96, 128, 256, 256, 256, 256], initializer=None):
        with tf.compat.v1.name_scope("Residual_Encoder"):
            if initializer is None:
                init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
            else:
                init = initializer
...
class Dilated_Encoder:
    def __init__(self, sequence, feature_map_count=[96, 128, 256, 256, 256, 256], initializer=None):
        with tf.compat.v1.name_scope("Dilated_Encoder"):
            if initializer is None:
                init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
            else:
                init = initializer
