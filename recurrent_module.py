    ...
    def __init__(self,  n_cells=4,  n_input=1024, n_hidden_state=128, initializer=None):
        with tf.compat.v1.name_scope("GRU_Grid"):
            ...
            if params["VIS"]["HISTOGRAMS"]:
                for i in range(3):
                    tf.compat.v1.summary.histogram("U[{}]".format(i), self.U[i])
                    tf.compat.v1.summary.histogram("b[{}]".format(i), self.b[i])
...
class LSTM_Grid:
    def __init__(self, n_cells=4, n_input=1024, n_hidden_state=128, initializer=None):
        with tf.compat.v1.name_scope("LSTM_Grid"):
            if initializer is None:
                init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
            else:
                init = initializer
...
class Weight_Matrices:
    ...
    def multiply_grid(self, x):
        with tf.compat.v1.name_scope("multiply_grid"):
            x_list = []
        ...
        return tf.transpose(a=tf.convert_to_tensor(value=x_list), perm=[3, 0, 1, 2, 4])
